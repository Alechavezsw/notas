...
-- ---------------------------------------------------------------------------
-- entradas_data (Entradas: kanban tareas y proyección por unidad)
-- ---------------------------------------------------------------------------
...
CREATE TRIGGER update_entradas_data_updated_at
  BEFORE UPDATE ON entradas_data
  FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
CREATE POLICY "Allow all operations on entradas_data" ON entradas_data FOR ALL USING (true) WITH CHECK (true);
